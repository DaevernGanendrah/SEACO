...
// import data2018 from './full_structured_health_data_2018_5.json'; // Ensure the file is correctly located
...
//           }
//         });
//         setData2018(organizedData);
//         setSubdistrictData(organizedData[selectedSubdistrict]);
//       })
//       .catch((error) => console.error('Error fetching 2018 data:', error));
//   }, [selectedSubdistrict]);

//   useEffect(() => {
//     // Update the subdistrict data when selected
//     if (data2018[selectedSubdistrict]) {
//       setSubdistrictData(data2018[selectedSubdistrict]);
//     }
//   }, [data2018, selectedSubdistrict]);
...
const HealthChart2018 = () => {
  ...
  // Fetch data from MongoDB for 2018
  useEffect(() => {
    const backendUrl = process.env.REACT_APP_BACKEND_URL || 'https://seaco.onrender.com';
    fetch(`${backendUrl}/api/health/2018`)
      .then((response) => response.json())
  ...
  useEffect(() => {
    // Update subdistrict data when selected
    if (data2018[selectedSubdistrict]) {
      setSubdistrictData(data2018[selectedSubdistrict]);
    }
  }, [data2018, selectedSubdistrict]);
  ...
  useEffect(() => {
    if (!mapRef.current) {
      ...
      // Fetch and display SEACO GeoJSON
      const backendUrl = process.env.REACT_APP_BACKEND_URL || 'https://seaco.onrender.com';
      fetch(`${backendUrl}/api/seaco-geojson`)
        .then((response) => response.json())
        .then((data) => {
          L.geoJSON(data, {
            style: {
